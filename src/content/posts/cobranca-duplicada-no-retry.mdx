...
A janela some porque :marca[verificar e gravar passam a ser a mesma operação]. Role devagar: a lousa acompanha a leitura e avança um passo a cada parágrafo.

<LousaPassos desenho="cobranca-duplicada-no-retry/passos" rotulo="Diagrama: o app, o serviço, o adquirente e a tabela cobranca. A segunda tentativa, com a mesma chave, bate na restrição única e recebe a resposta guardada, sem nova cobrança.">
  <Passo>O app pede a cobrança de R\$ 250 e manda junto a chave de idempotência que ele gerou, `5b0e7a52…`. A chave é a mesma em todas as tentativas desse pedido.</Passo>
  <Passo>Antes de cobrar, o serviço grava a chave na tabela `cobranca`, que tem restrição `UNIQUE` sobre ela. Quem grava primeiro fica com a chave, e o status começa como `PROCESSANDO`.</Passo>
  <Passo>O serviço confirma o registro, cobra no adquirente e guarda a resposta junto da chave: `CAPTURADA`, com o `201` que devolveu ao app.</Passo>
  <Passo>A resposta se perde no caminho de volta. Para o app, é como se nada tivesse acontecido, então ele tenta de novo, com a mesma chave.</Passo>
  <Passo>O `INSERT` da segunda tentativa bate na restrição `UNIQUE`. Em vez de cobrar de novo, o serviço devolve a resposta que já estava guardada.</Passo>
</LousaPassos>
