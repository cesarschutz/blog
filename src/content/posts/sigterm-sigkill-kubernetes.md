...
- **`SIGTERM` (15)** — sinal que o processo pode capturar. Ao recebê-lo, a aplicação faz um *graceful shutdown* (encerramento gracioso): :marca[para de aceitar conexões novas, termina as requisições em andamento], fecha conexões com o banco, libera locks distribuídos e grava buffers e métricas pendentes.
- **`SIGKILL` (9)** — sinal que não pode ser capturado, bloqueado nem ignorado. :marca[O kernel encerra o processo na hora e nenhuma rotina de limpeza roda.] O risco é perder dados, deixar conexões órfãs no banco e transações pela metade.

## O ciclo de término de um pod

Um pod entra em `Terminating` quando é apagado: delete manual, rolling update, redução de réplicas, drenagem de nó (`kubectl drain`), eviction (despejo do pod pelo cluster) ou um Job que atingiu o `activeDeadlineSeconds` (veja o post sobre [CronJob e tempo máximo de execução](/posts/kubernetes-cronjob-concorrencia/)). Nesse momento, o API server registra o prazo de término e :marca[**duas trilhas começam em paralelo**]:
...
1. **Começa a contagem do `terminationGracePeriodSeconds`** (padrão de 30s). Ela começa **antes** do `preStop`, e não depois do `SIGTERM`: :marca[o mesmo prazo cobre o `preStop` e o shutdown do app]. É um orçamento único, não um tempo a mais para cada etapa.
...
3. **Trilha de container** — o kubelet executa o `preStop` hook, se houver, e :marca[só depois que ele termina manda o `SIGTERM`] ao processo principal (PID 1) de cada container. Se a imagem definir outro sinal com a instrução `STOPSIGNAL`, os runtimes costumam enviar esse sinal no lugar do `SIGTERM`.
4. **Fim do prazo** — se algum container ainda estiver rodando quando o grace period acabar, :marca[o runtime envia `SIGKILL`]. Se o `preStop` ainda estiver rodando nessa hora, o kubelet concede uma única extensão de 2 segundos antes de forçar.
...
Como as duas trilhas são paralelas, há uma *race condition* (condição de corrida): durante alguns segundos o app já recebeu o `SIGTERM` e começou a se desmontar, mas :marca[o ingress ou o load balancer **ainda não soube** que o pod está saindo] e continua mandando requisições para ele. Essas requisições falham.

A prática recomendada, inclusive pela documentação do Spring Boot, é :marca[um `preStop` que apenas espera alguns segundos antes do `SIGTERM`]. A partir do Kubernetes 1.30 existe a ação `sleep` nativa (estável desde a 1.34), que não depende de shell na imagem:
...
O `sleep` não drena conexões: :marca[ele só **atrasa o `SIGTERM`**], dando tempo para o tráfego novo parar de chegar antes de o app começar a se desmontar. Não há um valor universal. Os 15s do exemplo são um ponto de partida: meça quanto tempo o seu ingress ou load balancer leva para tirar o pod da rotação.

## Exit codes úteis para diagnóstico

Quando um processo morre por causa de um sinal, o código de saída segue a convenção :marca[**128 + número do sinal**]. Ele aparece no status do container (`kubectl describe pod`, nos campos `State` e `Last State`) e nas ferramentas de monitoramento:

- `0` — o processo terminou por conta própria com sucesso (chamou `exit(0)`).
- `143` (128 + 15) — o processo terminou por causa do `SIGTERM`. Numa aplicação Java, :marca[**esse é o resultado normal de um shutdown gracioso**]: a JVM executa os shutdown hooks (é assim que o Spring fecha o contexto) e depois sai com 143. Não indica falha.
...
Um 137 com o motivo `OOMKilled` aponta para memória; :marca[um 137 sem esse motivo, logo após um deploy, costuma indicar que o shutdown não coube no grace period].

## Spring Boot — graceful shutdown nativo

O Spring Boot suporta graceful shutdown desde a versão 2.3, e :marca[desde a **3.4 ele vem ativado por padrão**]. No Spring Boot 4 ele funciona com os três servidores web embarcados suportados: Tomcat, Jetty e Reactor Netty. O Undertow deixou de ser suportado no Spring Boot 4; nas versões 2.3 a 3.x ele também tinha graceful shutdown, mas respondia `503` às requisições novas em vez de recusá-las na camada de rede.
...
2. As requisições em andamento têm até `timeout-per-shutdown-phase` para terminar. Como o nome diz, :marca[esse limite vale **por fase** de encerramento], e não para o shutdown inteiro.
3. O contexto termina de fechar: pools de conexão, schedulers e os métodos `@PreDestroy` dos beans.

Atenção ao testar localmente: :marca[parar o app pela IDE pode encerrar o processo sem mandar um `SIGTERM` de verdade], e aí o shutdown não é gracioso.
...
Esse tempo de espera (até 10s aqui) :marca[**soma-se** ao tempo do servidor web] e também precisa caber no grace period.
...
:marca[O shutdown completo do Spring precisa caber no grace period do Kubernetes], que também cobre o `preStop`. Some o pior caso de cada etapa e deixe uma folga:
...
Se a soma passar do `terminationGracePeriodSeconds`, :marca[o Kubernetes manda `SIGKILL` **no meio do shutdown**], exatamente o que se queria evitar. Ao aumentar o `timeout-per-shutdown-phase`, aumente também o grace period.

## A pegadinha do PID 1

O kubelet manda o `SIGTERM` só ao PID 1 do container. Para o Spring recebê-lo, :marca[o processo `java` precisa **ser** o PID 1] ou receber o sinal repassado por ele. O erro clássico está no `ENTRYPOINT`:
...
- **`OOMKilled`** — o kernel mata o container que passou do limite de memória com `SIGKILL`. :marca[Não há `SIGTERM` nem grace period.]
