...
Rotinas agendadas no Kubernetes costumam sofrer com três problemas clássicos: :marca[**execuções duplicadas, novas tentativas perigosas e jobs travados**]. Alguns campos de `CronJob` e `Job` existem justamente para evitá-los. Este post explica cada um, com seus valores padrão, mostra como eles se combinam numa configuração conservadora e aponta o que continua sendo responsabilidade da aplicação.
...
Isso importa principalmente quando o processamento altera dados, envia eventos, faz conciliação, processa pagamentos ou recalcula saldos, tarefas que :marca[**não devem produzir o mesmo efeito duas vezes**].
...
Com `Forbid`, a regra é :marca[*"se o Job anterior ainda está rodando, não inicia outro"*]. É a opção mais segura quando a rotina não pode rodar duas vezes ao mesmo tempo.

**Exemplo prático:** um job de conciliação roda a cada 10 minutos. Se uma execução demorar 15 minutos com o padrão `Allow`, :marca[duas conciliações rodam juntas], com risco de duplicidade, disputa pelos mesmos dados e processamento fora de ordem. Com `Forbid`, a execução das 10:10 é pulada enquanto a das 10:00 estiver ativa.

Um detalhe: a política vale só para os Jobs criados :sublinhado[**pelo mesmo CronJob**]. Dois CronJobs diferentes que chamam a mesma rotina continuam podendo rodar em paralelo.

## `activeDeadlineSeconds`

Pertence ao `Job`. Define por quanto tempo o Job pode ficar ativo. Com `300`, são no máximo 5 minutos; passou disso, :marca[o Kubernetes encerra todos os Pods do Job e marca o Job como falho] (condição `Failed`, motivo `DeadlineExceeded`). Não há padrão: se o campo não for definido, o Job não tem limite de tempo.

Dois cuidados:

- O tempo conta a partir do início do Job (campo `status.startTime`), não do container, e vale para o Job inteiro, não importa quantos Pods ele crie. :marca[Tempo esperando o Pod ser agendado ou a imagem ser baixada também entra na conta.]
...
Este campo é o complemento indispensável do `Forbid`: se um Job travar e nunca terminar, o `Forbid` pula todos os ciclos seguintes. Sem `activeDeadlineSeconds`, :marca[um único Job travado **bloqueia a agenda inteira**].
...
Atenção ao nível: o spec do Pod também tem um campo `activeDeadlineSeconds`. :marca[O que limita a execução inteira é o do spec do Job (`jobTemplate.spec`).]

## `startingDeadlineSeconds`

Pertence ao `CronJob`. :marca[Define por quantos segundos, depois do horário agendado, o Kubernetes ainda aceita criar o Job.] Com `60`: *"se estava agendado para 10:00, o Job só pode ser criado até 10:01; depois disso, a execução é pulada"*. As próximas continuam agendadas normalmente. Não há padrão: sem o campo, uma execução atrasada não tem prazo para começar.
...
Pertence ao `CronJob` e é estável desde o Kubernetes 1.27. Sem ele, :marca[o `schedule` é interpretado no fuso horário do processo `kube-controller-manager`], que nem sempre é o que você imagina. Para uma janela como "02:00 no horário de Brasília", declare o fuso explicitamente com um nome da base de fusos IANA:
...
Não use `CRON_TZ=` ou `TZ=` dentro do `schedule`: :marca[o Kubernetes rejeita o recurso com erro de validação].

## `backoffLimit`

Pertence ao `Job`. Define quantas novas tentativas o Kubernetes faz depois de uma falha antes de marcar o Job como falho. O padrão é :circulo[**6**]. Com `backoffLimit: 3`, o Job aceita até 3 novas tentativas, com intervalo crescente entre elas (10 s, 20 s, 40 s… limitado a 6 minutos).
...
- Tentar de novo :marca[**sem saber se a execução anterior falhou antes ou depois de gravar dados**].

Em jobs sensíveis, muitas vezes é melhor falhar de forma clara, gerar log e alerta e deixar uma pessoa (ou um processo controlado) decidir o reprocessamento. :marca[O `activeDeadlineSeconds` tem prioridade sobre o `backoffLimit`]: ao atingir o tempo limite, o Job falha mesmo que ainda restem tentativas.

## `restartPolicy: Never`

Pertence ao template do Pod. Num Pod comum, o padrão é `Always`, mas :marca[em Jobs esse valor não é aceito]: só `Never` ou `OnFailure`, e por isso o campo precisa ser declarado.
...
A combinação `restartPolicy: Never` + `backoffLimit: 0` dá o comportamento mais previsível: *"o container falhou, o Pod falhou, o Job falhou. Não tenta de novo."* :marca[Nenhuma nova tentativa automática escondida.] A própria documentação sugere `Never` ao depurar Jobs: com `OnFailure`, o Pod é encerrado quando o limite de tentativas é atingido, o que dificulta ler a saída da execução.
